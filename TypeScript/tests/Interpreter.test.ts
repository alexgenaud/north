...
describe('Interpreter', () => {
    ...
    const assertExecuteStack = (execStr: string, expectStack: string, message?: string) => {
        if (message === undefined) {
            message = `execute(${execStr}) expect stack: ${expectStack}`;
        }
        interpreter.execute(execStr);
        expect(interpreter.stack.toString()).toBe(expectStack);
    };

    const assertExecutePop = (execStr: string, expectPop: number, message?: string) => {
        if (message === undefined) {
            message = `execute(${execStr}) expect pop: ${expectPop}`;
        }
        interpreter.execute(execStr);
        const actual = interpreter.stack.pop();
        //expect(interpreter.stack.pop()).toBe(expectPop);
        expect(actual).toBe(expectPop);
    };
    ...
    test('interpreter_execute_push', () => {
        assertExecutePop("1", 1)
    });

    test('interpreter_swap_one_line_7_9_swap', () => {
        assertExecuteStack("7 9 SWAP", "9 7")
    });

    test('interpreter_swap_push2_execute', () => {
        assertExecuteStack("5 8 SWAP", "8 5")
    });

    test('interpreter_add_one_line', () => {
        assertExecutePop("19 3 4 +", 7)
    });

    test('interpreter_add_push2_execute_plus', () => {
        assertExecutePop("5 8 +", 13)
    });

    test('interpreter_subtract_simple', () => {
        assertExecutePop("9 6 -", 3)
        assertExecutePop("9 16 -", -7)
    });

    test('interpreter_subtract_negative_one_liner', () => {
        assertExecutePop("7 11 -", -4)
    });

    test('interpreter_subtract_push2_execute_subtract', () => {
        assertExecutePop("9 6 -", 3)
    });

    test('interpreter_divide_int', () => {
