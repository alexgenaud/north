...
export default class Dictionary {
    ...
    public addWordColonFunctionAddressList(
        word: string,
        action: number[]
    ): number {
        const data: Data = new Data(action, DataType.fn_colon_norm);
        return this.addWordData(word, data);
    }

    public addWordCoreConditional(
        word: string,
        action: (m: Machine) => void
    ): number {
        const data: Data = new Data(action, DataType.fn_core_condition);
        return this.addWordData(word, data);
    }

    public addWordCoreImmediate(
        word: string,
        action: (m: Machine) => void
    ): number {
        const data: Data = new Data(action, DataType.fn_core_immediate);
        return this.addWordData(word, data);
    }

    public addWordCoreFunction(
        word: string,
        action: (m: Machine) => void
    ): number {
        const data: Data = new Data(action, DataType.fn_core_norm);
        return this.addWordData(word, data);
    }
    ...
    public addWordI32(word: string, action: number): number {
        const data: Data = new Data(action, DataType.integer);
        return this.addWordData(word, data);
    }
        ...
        return (m: Machine) => {
            ...
            for (const token of m.compile_definition) {
                ...
                if (address === null && isInt(token)) {
                    ...
                    const newAddress = this.addWordI8(
                        token,
                        parseInt(token, 10)
                    );
            ...
            this.addWordColonFunctionAddressList(
                wordName as string,
                addressList
    ...
    public varInitWord(m: Machine): void {
        ...
        const addressOfValue = this.machine.write(
            new Data(0, DataType.integer)
        );
        this.addWordI16(m.opstack.pop(), addressOfValue);
        m.costack.toggle(Mode.EXECUTE);
    }
    ...
    private varSetValAtAddress(dictionary: Dictionary): (m: Machine) => void {
        ...
        return (m: Machine) => {
            ...
            if (addressOfVar == null) {
                throw new Error(
                    "Unexpected address: ${addressOfVar} of variable: ${varName} with value: ${value}"
                );
            }
            this.machine.overwrite(
                new Data(value, DataType.integer),
                addressOfVar
            );
        };
    }
    ...
    private varGetValAtAddress(dictionary: Dictionary): (m: Machine) => void {
        ...
        return (m: Machine) => {
            ...
            if (data == null || !data.isInt()) {
                throw new Error(
                    `Must be a valid integer at address: ${addressOfVar} was data: ${data}`
                );
            }
    ...
    private loadCoreWords(): void {
        ...
        this.addWordCoreFunction("DUP", (m: Machine) =>
            m.opstack.push(m.opstack.peek())
        );
        this.addWordCoreFunction("NOT", (m: Machine) =>
            m.opstack.push(~m.opstack.pop())
        );
        this.addWordCoreFunction("ABS", this.absoluteValue);
        this.addWordCoreFunction("=0", (m: Machine) =>
        ...
        this.addWordCoreFunction("PM", (m: Machine) =>
            console.log(this.machine)
        );

        // 2 pop pops
        this.addWordCoreFunction("+", (m: Machine) =>
        ...
        this.addWordCoreFunction("OVER", (m: Machine) =>
            m.opstack.push(m.opstack.peek(-2))
        );
        this.addWordCoreFunction("SWAP", (m: Machine) =>
            m.opstack.push(m.opstack.pop(-2))
        );
        this.addWordCoreFunction("AND", (m: Machine) =>
            m.opstack.push(m.opstack.pop() & m.opstack.pop())
        );
        this.addWordCoreFunction("OR", (m: Machine) =>
            m.opstack.push(m.opstack.pop() | m.opstack.pop())
        );
        this.addWordCoreFunction("XOR", (m: Machine) =>
            m.opstack.push(m.opstack.pop() ^ m.opstack.pop())
        );
        this.addWordCoreFunction("<=0", (m: Machine) =>
            m.opstack.push(m.opstack.pop() <= 0 ? 1 : 0)
        ...
        // 3 pop pop pops
        this.addWordCoreFunction("ROT", (m: Machine) =>
