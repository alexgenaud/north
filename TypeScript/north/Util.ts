...
export function assertInt(wordName: string, value: any) {
    if (!isInt(value)) {
        throw new Error (`${wordName} expects integer but type ${(typeof value)} with value ${value}`);
    }
}

export function assertIntRangeInclusive(wordName: string, value: any, lowest: number, highest: number) {
    if (!isInt(value)) {
        throw new Error (`${wordName} expects integer but type ${(typeof value)} with value ${value}`);
    }
    if (value < lowest) {
        throw new Error (`${wordName} expects integer >= ${lowest} but value ${value}`);
    }
    if (value > highest) {
        throw new Error (`${wordName} expects integer <= ${highest} but value ${value}`);
    }
}
...
export function assertFuncIn(m: Machine, wordName: string, expectIn: number) {
    if (m.inputBuffer.length < expectIn) {
        throw new Error (`${wordName} expects inputBuffer >= ${expectIn} but only ${m.inputBuffer.length}`);
    }
}

export function assertFuncOp(m: Machine, wordName: string, expectOp: number) {
    if (m.opstack.length < expectOp) {
        throw new Error (`${wordName} expects opstack >= ${expectOp} but only ${m.opstack.length}`);
    }
}

export function assertFuncOpIn(m: Machine, wordName: string, expectOp: number, expectIn: number) {
    ...
    if (m.opstack.length < expectOp && m.inputBuffer.length < expectIn) {
        throw new Error (`${wordName} expects opstack, inputBuffer >= ${expectOp}, ${expectIn} but only ${m.opstack.length}, ${m.inputBuffer.length}`);
    }
    if (m.opstack.length < expectOp) {
        throw new Error (`${wordName} expects opstack >= ${expectOp} but only ${m.opstack.length}`);
    }
